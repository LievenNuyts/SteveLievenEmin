...
public class TextPersistenty implements Persistencable {
	...
	public void deleteFromQuiz(ChangeQuizView view, List<Quiz> tempQuizzes, List<Exercise> tempExercises){
		try {
			...
			int tempExId = view.getSelectedQuizExerciseValueFromList().getExercise().getExerciseId();
			
			for (Quiz q : tempQuizzes){
				if (q.getQuizId() == tempQuId)
					q.getQuizExercises().remove(view.getSelectedQuizExerciseValueFromList());
			}
			
			for (Exercise ex : tempExercises){
				if (ex.getExerciseId() == tempExId)
					ex.getQuizExercises().remove(view.getSelectedQuizExerciseValueFromList());
			}
		}
		catch(IllegalArgumentException ex){
			System.out.println(ex);
	...
	public void addToQuiz(ChangeQuizView view, List<Quiz> tempQuizzes, List<Exercise> tempExercises, 
		...
		try {
			...
			for (Exercise ex : tempExercises){
				if (ex.getExerciseId() == view.getSelectedExerciseValueFromList().getExerciseId()){
			...
			for(Quiz q : tempQuizzes){
				if(q.getSubject().equalsIgnoreCase(view.getSelectedQuizValueFromList().getSubject())){
					
					int indexPos = controller.getIndexPosition(q);
					System.out.println(indexPos);
					tempQuizzes.set(indexPos, view.getSelectedQuizValueFromList());
				}
			}
		} 
		catch(IllegalArgumentException ex){
